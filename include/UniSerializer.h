...
#endif 
#endif /*__TRIM__*/
#ifdef UNISERIALIZER_IMPLEMENTATION
static UniCallbacks callbacks = { malloc, realloc, free };
int Uni_is_big_endian(void)
{
	union {
		uint32_t i;
		char c[4];
...
UniBuffer Uni_init(size_t InitialSize, bool autoResize, bool isBigEndian) {
	...
	// to be reworked
	ctx.systemBigEndian = Uni_is_big_endian();
...
bool Uni_ResizeIfNeed(UniBuffer* ctx, size_t Size) {
...
bool Uni_VerifyEntrySize(UniBuffer* ctx, size_t Size) {
	return (ctx->position + Size) > ctx->length;
}
bool Uni_encode_8(UniBuffer* ctx, uint8_t value) {
	if (Uni_ResizeIfNeed(ctx, 1) == false) {
		return false;
	}
...
bool Uni_encode_16(UniBuffer* ctx, uint16_t value) {
	if (Uni_ResizeIfNeed(ctx, 2) == false) {
		return false;
	}
	if (Uni_is_big_endian()) {
		*(ctx->buffer + ctx->position) = value & 0xff;
...
bool Uni_encode_32(UniBuffer* ctx, uint32_t value) {
	if (Uni_ResizeIfNeed(ctx, 4) == false) {
		return false;
	}
...
bool Uni_encode_64(UniBuffer* ctx, uint64_t value) {
	if (Uni_ResizeIfNeed(ctx, 8) == false) {
		return false;
	}
	if (Uni_is_big_endian()) {
		*(ctx->buffer + ctx->position) = value & 0xff;
...
bool Uni_encode_Bytes(UniBuffer* ctx, uint8_t* value, size_t length)
{
	if (Uni_ResizeIfNeed(ctx, length) == false) {
		return false;
	}
...
uint8_t* Uni_decode_Bytes(UniBuffer* ctx, size_t length, uint8_t* output, bool* error)
{
	if (Uni_VerifyEntrySize(ctx, length)) {
		*error = true;
		return 0;
	}
	uint8_t* tmp;
